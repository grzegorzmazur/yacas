...
(defun ynb-get-maybe-error-message ()
  "Get an error message (maybe)."
  (let ((beg)
        (end))
    (save-excursion
      (set-buffer (process-buffer inferior-yacas-process))
      (goto-char (point-max))
      (search-backward yacas-input-prompt-string nil t 3)
      (forward-line 1)
      (setq beg (point))
      (search-forward yacas-input-prompt-string nil t)
;;      (search-backward yacas-input-prompt-string nil t)
      (beginning-of-line)
      (forward-char (- (length yacas-input-prompt-string)))
      (forward-char -1)
      (setq end (point))
...
separated by a ynb-output-marker line."
  ...
  (let ((cell)
    ...
    (while (string-match "[^ \t\n]" cell)
      ...
      (yacas-single-command ynb-tex-string)
      (setq cell (substring cell end))
      (setq out (yacas-tex-output))
      (setq jpbout out)
      (if (string-match "mathrm{\\$" out)
          (progn
            (goto-char pt2)
            (insert (ynb-get-maybe-error-message) "\n"))
        (goto-char pt2)
        (insert "\\out " (yacas-tex-output) ";\n"))
      (setq pt2 (point)))
    (goto-char pt)))
...
(if yacas-notebook-mode-map ()
  (let ((map (copy-keymap texmode-map)))
    ...
    (define-key map [(control return)] 'ynb-update-cell)
    (define-key map "\C-c\C-uC" 'ynb-tex-update-cell)
    (define-key map [(meta return)] 'ynb-tex-update-cell)
