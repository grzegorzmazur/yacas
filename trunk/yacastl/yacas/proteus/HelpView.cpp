//
// "$Id: HelpView.cpp,v 1.1 2006-02-17 13:08:41 peterdgilbert Exp $"
//
...
HelpView::draw()
{
  ...
  const char	*ptr,		// Pointer to text in block
		*attrs = NULL;		// Pointer to start of element attributes
...
//  printf("size = %d, height = %d, yy = %d, hh = %d\n",size_, h(),yy,hh);
...
//    printf("AYAL: going to file [%s] anchor [%s]\n",
//          temp, link->name);

    if (strcmp(temp, filename_) != 0 && link->filename[0])
    {
        load(temp);
    }
    else if (target[0])
...
: Fl_Group(xx, yy, ww, hh, l),
  value_(NULL),blocks_(NULL),link_(NULL),links_(NULL),
  targets_(NULL)
...
//
// End of "$Id: HelpView.cpp,v 1.1 2006-02-17 13:08:41 peterdgilbert Exp $".
//
