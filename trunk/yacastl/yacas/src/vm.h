...
#define BitsPerLong BITS_PER_LONG
  ...
  extern void RunFunction(LispEnvironment& aEnvironment,LispInt aStackTop,
            const unsigned char* code, RefPtr<LispObject> aConstants[]);
  ...
  #define CodeVmPush                0x05 // (_index)          //PUSH(_index)
  #define CodeVmPushConstant        0x06 // (_constnt) //PUSH(_constnt)
  #define CodeVmInitRegister        0x07 // (_index,_constant) // (_index) = (_constant)
  #define CodeVmSetRegister         0x08 // (_index)            // (_index) = (STACK(STACKTOP()-1))
