...
char* ReadInputString(char* prompt)
{
	...
	while (true)	// while 'help'
	{
		...
		if (inpline[0] == '?')
		{
			char * helpOn = &inpline[1];
			char buf[120];
			if (*helpOn == '?' || strlen(helpOn) >= 100)
				strcpy(buf, "Help()");
			else
				#ifdef HAVE_VSNPRINTF
					snprintf(buf,120,"Help(\"%s\")",helpOn);
				#else
					sprintf(buf,     "Help(\"%s\")",helpOn);
				#endif
			yacas->Evaluate(buf);
			continue;
		}
		else if (!strncmp(inpline,"restart",7))
		{
			restart=LispTrue;
			busy=LispFalse;
		}
...
#undef CORE_KERNEL_FUNCTION
    ...
    if (archive)
    {
        ...
        else
        {
            ...
            unsigned char* fullbuf = PlatAllocN<unsigned char>(fullsize);
            if (fullbuf)
            {
            ...
            if(FD_ISSET(fd,&testfds))
            {
                if(fd == server_sockfd)
                {
...
#endif
                    {
                        ...
                        DBG_printf("adding client on fd %d\n", client_sockfd);
                    }
                }
                else
                {
                    ...
                    if(nread == 0)
                    {
                        ...
                        nrSessions--;
                        DBG_printf("Removing client on %d\n",fd);
                    }
                    else
                    {
//                      LispString cmd;
...
/*
                        ...
                        cmd.Append('\0');                        
*/
DBG_printf("Servicing on %ld (%ld)\n",(long)fd,(long)used_clients[clsockindex]);

                        if (clsockindex < maxConnections)
                        {
                            if (!used_clients[clsockindex])
                            {
								DBG_printf("Loading new Yacas environment\n");
...
//printf("sizeof(LispPtr) = %d\n",sizeof(LispPtr));
	const bool showsize = false DBG_( || verbose_debug );
    if (showsize)
	{
		...
		SHOWSIZE(LispNumber);
			SHOWSIZE(LispObject);
			SHOWSIZE(RefPtr<BigNumber>);
			SHOWSIZE(RefPtr<LispString>);
...
//              show_prompt = 0;
        ...
        else if (!strcmp(argv[fileind],"--read-eval-print"))
        {
			...
			//printf("rep %d\n",argv[fileind][0]);
            if (fileind<argc)
				read_eval_print = *argv[fileind] ? argv[fileind] : 0;
        }
    ...
    /*
        printf("[%d %d]\n",LispHash("\"a\""), LispHashStringify( "a" ));
    */
...
// define STD_COMMANDLINE if you want the standard command line always
#ifdef STD_COMMANDLINE
	use_plain = 1;
#endif
	if (use_plain)
		commandline = NEW CStdCommandLine;
	else
		commandline = NEW FANCY_COMMAND_LINE;	// WIN32 ? CWin32CommandLine : CUnixCommandLine
    ...
    for ( ; fileind<argc; fileind++)
    {
        char s[200];
#ifdef HAVE_VSNPRINTF
		snprintf(s,200,"%sLoad(\"%s\");",patchload?"Patch":"",argv[fileind]);
#else
		sprintf(s,     "%sLoad(\"%s\");",patchload?"Patch":"",argv[fileind]);
#endif
        yacas->Evaluate(s);
        if (yacas->IsError())
        {
