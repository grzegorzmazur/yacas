...
\maketitle
\begin{abstract}
I describe and compare several methods for arbitrary-precision numerical
computation of Chebyshev polynomials $T_{n}\left(x\right)$, $U_{n}\left(x\right)$
for complex $x$. I introduce a new algorithm based on Fibonacci-like
sequences and indicate the asymptotically most efficient methods in
the limits of large order $n$ and large required precision.
\end{abstract}
...
In the following sections I present several methods for computing
...
part for all other $x$. If $\textrm{Im}\,\ln z\neq0$, the function\[
...
if $\arg z$ is found to $P+\log_{10}\left|n\arg z\right|$ digits.
If $\left|z\right|\neq1$ (this happens for real $\left|x\right|<1$),
we also need to know $\left|z\right|$ to $P+\log_{10}n$ digits.
For $x$ not close to $1$, $\left|\arg z\right|$ is not small and
...
equivalent to that of $T_{n}\left(x\right)$. The case $x\approx1$
is considered below.
...
Again we assume that $x$ is not close to $1$ (otherwise there is
a cancellation in the formula for $U_{n}$). The calculation of the
$n$-th power can be performed in
...
$P+\log_{10}n-Q/2$ digits (relative accuracy). One computes $\ln z-1$
to $P+\log_{10}n-Q/2$ digits and then evaluates $\cosh\left(n\ln z\right)$
...
multiplications. The matrix form of the same relation,\begin{eqnarray}
\left(\begin{array}{c}
T_{n+1}\\
...
that of Eqs.~(\ref{eq:T0sqrt})-(\ref{eq:U0sqrt}) because matrix
multiplication takes longer than scalar multiplication. Therefore
the linear recurrence method offers no advantage.


\subsection{Quadratic recurrences and Fibonacci sequences}

The following quadratic recurrences can be used \cite{AS64}, \begin{align}
...
for $n=1$ or $n=0$ that do not require any computations. 

To avoid recursion, one can build a sequence of indices $k_{1}$,
$k_{2}$, ..., $n$, such that $T_{k_{i}}\left(x\right)$ for each
subsequent element $k_{i}$ can be computed using the recurrences
and the previously found values $T_{k}\left(x\right)$. For example,
the sequence (1, 2, 3, 5, 10, 15) directs one to compute first $T_{2}\left(x\right)$
...
($k_{1}$, ..., $k_{L}=n$). One method to build the sequence was
...
etc. The resulting chain is (1, 2, 3, 4, 5, 9, 10, 19, 20, 39). For
any $n$ the chain can be generated by examining the bits in the binary
representation of $n$. The bisection method gives $L\approx2\log_{2}n$
...
For example, for $n=39$ there is a shorter sequence (1, 2, 3, 6,
...
This is approximately the same as the coefficient in Eq.~(\ref{eq:Cost2}).
Therefore the method of quadratic recurrences does not improve on the
algebraic method, even when optimal sequences can be found.
...
I obtained explicit cost estimates for the described methods of arbitrary-precision
